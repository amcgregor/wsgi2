...
# The server must not attempt to handle byte range requests; the application can optimize this use case far more easily than a server.  (For example and application can generate the correct body length vs. generating the whole body and having the server buffer and slice it.)
